...
namespace team45
{
	void Histogram::calculate(std::vector<cv::Point3f>const& colors)
	{
		m_hist.resize(m_bins.size());
	...
	void Histogram::draw()
	{
		...
		{			
			rectangle(hist_image,
				...
				Point2f(bin_w * (i + 1.5f), hist_h - m_hist[i] * hist_h),
				Scalar(m_bins[i].x, m_bins[i].y, m_bins[i].z), -2, 8, 0);
		}
